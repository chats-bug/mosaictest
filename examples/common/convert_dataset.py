...
c4constants.splits['val_small'] = DataSplitConstants(hf_split='validation',
                                                     ...
                                                     raw_samples=10000,
                                                     truncated_samples=10000)
myconstants = DatasetConstants(
    chars_per_sample=120,  # Computed over validation set
    chars_per_token=4  # OpenAI estimate
)
myconstants.splits['train'] = DataSplitConstants(hf_split='train',
                                                 folder_split='train',
                                                 raw_samples=200,
                                                 truncated_samples=None)


CONSTS = {'c4': c4constants, 'the_pile': pileconstants, 'ayush-vatsal/description_to_caption': myconstants}
...
class NoConcatDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterable[Dict[str, bytes]]:
        for sample in self.hf_dataset:
            ...
            yield {'text': sample['Caption'].encode('utf-8')}
...
class ConcatTokensDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterable[Dict[str, bytes]]:
        ...
        buffer = []
        for sample in self.hf_dataset:
            encoded = self.tokenizer(sample['Caption'],
                                     truncation=False,
                                     padding=False)
...
def main(args: Namespace) -> None:
    ...
    for split_name in args.splits:
        ...
        with MDSWriter(columns=columns,
                       dirname=os.path.join(args.out_root, folder_split),
...
# python ../common/convert_dataset.py --dataset ayush-vatsal/description_to_caption --out_root ./my-copy-captions --splits train --concat_tokens 512 --tokenizer gpt2 --eos_text '<|endoftext|>'

# python ../common/convert_dataset.py --dataset my --out_root ./my-copy --splits train val --concat_tokens 2048 --tokenizer gpt2 --eos_text '<|endoftext|>'